<script lang="ts">
	import { onMount } from 'svelte';
	import { page } from '$app/state';
	import { goto } from '$app/navigation';
	import { resolve } from '$app/paths';
	import { t } from '$lib/i18n';
	import { api } from '$lib/api/client';
	import StatusBanner from '$lib/components/ui/StatusBanner.svelte';

	let isFinalizing = $state(false);
	let error = $state<string | null>(null);

	onMount(() => {
		const sessionId = sessionStorage.getItem('audible_session_id');
		const url = page.url.toString();
		
		// If we have a sessionId and are on the return path from Amazon
		if (sessionId && (url.includes('openid.oa2.authorization_code') || url.includes('code='))) {
			finalizeConnection(sessionId, url);
		}
	});

	async function finalizeConnection(sessionId: String, callbackUrl: string) {
		isFinalizing = true;
		const result = await api.post<any>('/podcasts/audible/finalize', {
			sessionId,
			callbackUrl
		});
		
		if (result.right) {
			sessionStorage.removeItem('audible_session_id');
			goto(resolve('/podcasts/audible/browse'));
		} else if (result.left) {
			error = result.left.message;
			isFinalizing = false;
		}
	}

	async function startConnect() {
		error = null;
		const result = await api.post<{ loginUrl: string, sessionId: string }>('/podcasts/audible/connect', {});
		if (result.right) {
			sessionStorage.setItem('audible_session_id', result.right.sessionId);
			window.location.href = result.right.loginUrl;
		} else if (result.left) {
			error = result.left.message;
		}
	}
</script>

<svelte:head>
	<title>{$t('audible.connect_title')} | Shelf</title>
</svelte:head>

<div class="mx-auto max-w-xl py-12">
	<div class="text-center mb-12">
		<div class="inline-flex h-20 w-20 items-center justify-center rounded-3xl bg-[#F5D000]/10 text-[#F5D000] mb-6">
			<svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12" viewBox="0 0 24 24" fill="currentColor">
				<path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm-1-13h2v6h-2zm0 8h2v2h-2z"/>
			</svg>
		</div>
		<h1 class="font-display text-4xl font-bold text-foreground">{$t('audible.connect_title')}</h1>
		<p class="mt-2 text-muted-foreground">{$t('audible.connect_subtitle')}</p>
	</div>

	<div class="rounded-3xl border border-border bg-card p-8 shadow-2xl">
		{#if error}
			<div class="mb-6">
				<StatusBanner kind="error" title="Connection Failed" message={error} />
			</div>
		{/if}

		{#if isFinalizing}
			<div class="flex flex-col items-center py-8">
				<div class="h-10 w-10 animate-spin rounded-full border-4 border-primary/30 border-t-primary mb-4"></div>
				<p class="font-medium text-foreground">{$t('audible.finalize_loading')}</p>
			</div>
		{:else}
			<div class="space-y-6">
				<p class="text-sm leading-relaxed text-muted-foreground text-center px-4">
					{$t('audible.login_instruction')}
				</p>

				<button
					type="button"
					onclick={startConnect}
					class="w-full rounded-2xl bg-[#F5D000] py-4 font-display text-lg font-bold text-black shadow-lg shadow-[#F5D000]/20 transition-all hover:bg-[#F5D000]/90 hover:scale-[1.02] active:scale-[0.98]"
				>
					{$t('audible.start_login')}
				</button>

				<a
					href={resolve('/podcasts')}
					class="block w-full py-2 text-center text-sm font-medium text-muted-foreground hover:text-foreground transition-colors"
				>
					Cancel and go back
				</a>
			</div>
		{/if}
	</div>
</div>
