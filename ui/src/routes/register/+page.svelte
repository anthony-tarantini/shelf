<script lang="ts">
	...
	async function handleSubmit() {
		...
		const result = await api.post<UserResponse>('/users', {
...
<AuthCard
	...
	subtitle={$t('register.page_subtitle')}
>
	<form onsubmit={(e) => { e.preventDefault(); handleSubmit(); }} class="space-y-4">
		...
		{#if error}
			<StatusBanner kind="error" title={$t('register.error_failed')} message={error} />
		{/if}
		...
		>
			{#if loading}
				...
				{$t('register.loading_creating_account')}
			{:else}
	...
	{#snippet footer()}
				<p class="text-muted-foreground text-sm">
					...
					<a href={resolve("/login")} class="text-primary hover:text-primary/80 font-medium">{$t('register.link_sign_in')}</a>
				</p>
	{/snippet}
</AuthCard>
