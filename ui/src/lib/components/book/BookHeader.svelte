...
<header class="mb-8 flex flex-col gap-5 rounded-[1.5rem] border border-border/70 bg-card/70 p-4 shadow-xl shadow-black/5 backdrop-blur-md sm:mb-10 sm:p-6 lg:flex-row lg:items-start lg:justify-between lg:gap-6">
    ...
    <div class="flex w-full gap-3 sm:w-auto">
        ...
        >
            {editMode ? $t('common.actions.cancel') : $t('books.header.edit_metadata')}
        </button>
        {#if onFetchMetadata}
            <button
                    onclick={onFetchMetadata}
                    class="w-full rounded-xl border border-border bg-accent px-5 py-3 font-bold text-foreground transition-all hover:bg-accent/80 sm:w-auto sm:px-6"
            >
                {metadataMode ? $t('common.actions.cancel') : $t('books.header.fetch_metadata')}
            </button>
        {/if}
    </div>
</header>
