    ...
    IntegrationSpec({
        "getUserSettings should return default settings if none exist" {
            testWithDeps { deps ->
                ...
                val settingsService = settingsService(deps.database.settingsQueries)
        ...
        "updateUserSettings should persist and retrieve settings" {
            testWithDeps { deps ->
                ...
                recover({
                    ...
                    settingsService.updateUserSettings(user.id.id, true)
                    val settings = settingsService.getUserSettings(user.id.id)
                    settings.syncMetadataToFiles shouldBe true

                    settingsService.updateUserSettings(user.id.id, false)
