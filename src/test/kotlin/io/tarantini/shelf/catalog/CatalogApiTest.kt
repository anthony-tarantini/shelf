...
class CatalogApiTest :
    IntegrationSpec({
        ...
        "series update with null title keeps existing title" {
            testApp { client ->
                val token =
                    registerUser(client, "catalog-series-null@example.com", "catalogseriesnull")
                val uniqueTitle = "Hyperion ${Uuid.random()}"

                val createResponse =
                    client.post("/api/series") {
                        header(HttpHeaders.Authorization, "Bearer $token")
                        contentType(ContentType.Application.Json)
                        setBody(Request(SeriesRequest(title = uniqueTitle)))
                    }
                createResponse.status shouldBe HttpStatusCode.Created
                val created = createResponse.body<Response<SavedSeriesRoot>>().data
                val seriesId = created.id.id.value.toString()

                val updateResponse =
                    client.put("/api/series/$seriesId") {
                        header(HttpHeaders.Authorization, "Bearer $token")
                        contentType(ContentType.Application.Json)
                        setBody(Request(SeriesRequest(title = null)))
                    }
                updateResponse.status shouldBe HttpStatusCode.OK
                updateResponse.body<Response<SavedSeriesRoot>>().data.name shouldBe uniqueTitle
            }
        }
        ...
        "series cover endpoint returns not found when no covered books exist" {
            testApp { client ->
                val token = registerUser(client, "series-cover-miss@example.com", "seriescovermiss")
                var seriesIdString = ""
                testWithDeps { deps ->
                    recover({
                        with(deps.database) {
                            val seriesId = seriesQueries.createSeries("Series Without Cover")
                            seriesIdString = seriesId.value.toString()
                        }
                    }) {
                        fail("Seeding failed: $it")
                ...
                val response =
                    client.get("/api/series/$seriesIdString/cover") {
                        header(HttpHeaders.Authorization, "Bearer $token")
                    }
        ...
        "series cover endpoint returns image content with cache headers" {
            testApp { client ->
                val token =
                    registerUser(client, "series-cover-bytes@example.com", "seriescoverbytes")
                var seriesIdString = ""
                val coverPath = StoragePath.fromRaw("books/series-cover-api/cover.jpg")
                testWithDeps { deps ->
                    recover({
                        with(this) {
                            deps.storageService.save(coverPath, FileBytes(byteArrayOf(1, 2, 3)))
                        }
                        with(deps.database) {
                            val bookId = bookQueries.createBook("Series Cover API Book", coverPath)
                            val seriesId = seriesQueries.createSeries("Series Cover API")
                ...
                response.contentType()?.withoutParameters()?.toString() shouldBe "image/jpeg"
            }
        }

        "series books endpoint returns summaries from provider projection" {
            testApp { client ->
                val token = registerUser(client, "series-books@example.com", "seriesbooks")
                var seriesIdString = ""
                var seededTitle = ""
                testWithDeps { deps ->
                    recover({
                        with(deps.database) {
                            seededTitle = "Provider Projection Book"
                            val bookId = bookQueries.createBook(seededTitle, null)
                            val seriesId = seriesQueries.createSeries("Provider Projection Series")
                            bookQueries.linkSeries(bookId, seriesId, 1.0)
                            seriesIdString = seriesId.value.toString()
                        }
                    }) {
                ...
                val response =
                    client.get("/api/series/$seriesIdString/books") {
                        header(HttpHeaders.Authorization, "Bearer $token")
                    }

                response.status shouldBe HttpStatusCode.OK
                val books = response.body<Response<List<BookSummary>>>().data
                books.any { it.title == seededTitle } shouldBe true
            }
        }
