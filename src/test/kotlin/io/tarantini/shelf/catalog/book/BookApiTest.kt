...
import io.tarantini.shelf.catalog.book.domain.BookPage
...
class BookApiTest :
    IntegrationSpec({
        ...
        "books page should support server-side filters" {
            testApp { client ->
                val token = registerUser(client, "book-filters@example.com", "bookfilters")
                var duneBookId = ""
                var foundationBookId = ""
                testWithDeps { deps ->
                    recover({
                        with(deps.database) {
                            val duneBook = bookQueries.createBook("Dune", null)
                            val foundationBook = bookQueries.createBook("Foundation", null)
                            val frank = authorQueries.createAuthor("Frank Herbert")
                            val isaac = authorQueries.createAuthor("Isaac Asimov")
                            val duneSeries = seriesQueries.createSeries("Dune Saga")
                            val foundationSeries = seriesQueries.createSeries("Foundation Series")

                            bookQueries.insertBookAuthor(duneBook, frank)
                            bookQueries.insertBookAuthor(foundationBook, isaac)
                            bookQueries.linkSeries(duneBook, duneSeries, 1.0)
                            bookQueries.linkSeries(foundationBook, foundationSeries, 1.0)

                            duneBookId = duneBook.value.toString()
                            foundationBookId = foundationBook.value.toString()
                        }
                    }) {
                        fail("Seeding failed: $it")
                    }
                ...
                client.put("/api/books/$duneBookId/status") {
                    header(HttpHeaders.Authorization, "Bearer $token")
                    contentType(ContentType.Application.Json)
                    setBody(Request(ReadStatusRequest(ReadStatus.READING)))
                }

                client.put("/api/books/$foundationBookId/status") {
                    header(HttpHeaders.Authorization, "Bearer $token")
                    contentType(ContentType.Application.Json)
                    setBody(Request(ReadStatusRequest(ReadStatus.FINISHED)))
                }

                val filteredResponse =
                    client.get(
                        "/api/books/page?page=0&size=20&title=Dune&author=Frank&series=Saga&status=READING"
                    ) {
                        header(HttpHeaders.Authorization, "Bearer $token")
                    }
                filteredResponse.status shouldBe HttpStatusCode.OK

                val page = filteredResponse.body<Response<BookPage>>().data
                page.totalCount shouldBe 1
                page.items.size shouldBe 1
                page.items.first().book.title shouldBe "Dune"
            }
        }
