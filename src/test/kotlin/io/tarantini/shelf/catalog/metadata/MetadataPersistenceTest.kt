...
class MetadataPersistenceTest :
    IntegrationSpec({
        "saveAggregate and getMetadataByBookId should persist and retrieve a complete metadata aggregate" {
            testWithDeps { deps ->
                val repository = metadataRepository(deps.database.metadataQueries)
                ...
                recover({
                    ...
                    val metadataId = repository.saveAggregate(aggregate)

                    val fetched = repository.getMetadataByBookId(bookId)
                    ...
                    val fetchedEditions = repository.getEditionsByBookId(bookId)
                    ...
                    val fetchedChapters = repository.getChaptersByEditionId(fetchedEdition.id.id)
        ...
        "getMetadataForBooks should return a map for multiple book IDs" {
            testWithDeps { deps ->
                ...
                recover({
                    ...
                    val map = repository.getMetadataForBooks(listOf(bookId1, bookId2))
                    map.size shouldBe 2
        ...
        "deleteMetadataByBookId should remove all associated metadata" {
            testWithDeps { deps ->
                ...
                recover({
                    ...
                    repository.deleteMetadataByBookId(bookId)

                    recover({
                        repository.getMetadataByBookId(bookId)
