...
import io.tarantini.shelf.processing.import.domain.MergeStagedBookRequest
            ...
            testWithDeps { deps ->
                ...
                recover({
                    ...
                    with(this) {
                        with(auth) {
                            with(deps.database) {
                                deps.stagedBookService.promote(promoteStagedBook(stagedA.id))
                            }
                        }
                    }
                    ...
                    with(this) {
                        with(auth) {
                            deps.stagedBookService.update(
                                stagedId = stagedB.id,
                                command =
                                    UpdateStagedBookRequest(
                                            audiobookMetadata =
                                                StagedEditionMetadata(
                                                    storagePath = "different/path.m4b"
                                                )
                                        )
                                        .toCommand(),
                            )
                    ...
                    with(this) {
                        with(auth) {
                            with(deps.database) {
                                deps.stagedBookService.merge(
                                    stagedB.id,
                                    MergeStagedBookRequest(bookA.id.value.toString()).toCommand(),
                                )
                            }
                        }
                    }
