...
fun settingsService(queries: SettingsQueries): SettingsService = object : SettingsService {
    override suspend fun getUserSettings(userId: UserId): UserSettingsRoot = withContext(Dispatchers.IO) {
        queries.getUserSettings(userId).executeAsOneOrNull()?.toRoot()
            ?: UserSettingsRoot(userId, false)
    }

    override suspend fun updateUserSettings(userId: UserId, syncMetadataToFiles: Boolean): UserSettingsRoot = withContext(Dispatchers.IO) {
