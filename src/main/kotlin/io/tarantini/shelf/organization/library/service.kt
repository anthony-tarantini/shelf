...
interface LibraryModifier {
    context(_: RaiseContext, auth: JwtContext)
    suspend fun createLibrary(command: CreateLibraryCommand): SavedLibraryRoot

    context(_: RaiseContext, auth: JwtContext)
    suspend fun updateLibrary(command: UpdateLibraryCommand): SavedLibraryRoot
...
fun libraryService(
    ...
    mutationRepository: LibraryMutationRepository = libraryMutationRepository(libraryQueries),
): LibraryService = LibraryAggregateService(libraryQueries, bookQueries, mutationRepository)

private class LibraryAggregateService(
    private val libraryQueries: LibraryQueries,
    private val bookQueries: BookQueries,
    private val mutationRepository: LibraryMutationRepository,
) : LibraryService {
    context(_: RaiseContext, auth: JwtContext)
    private suspend fun requireOwnedLibrary(id: LibraryId): SavedLibraryRoot {
        val library = mutationRepository.getLibraryById(id)
    ...
    context(_: RaiseContext, auth: JwtContext)
    override suspend fun createLibrary(command: CreateLibraryCommand): SavedLibraryRoot {
        val decision = LibraryMutationDecider.decideCreate(command)
        return mutationRepository.createLibrary(auth.userId, decision.title.value)
    }

    context(_: RaiseContext, auth: JwtContext)
    override suspend fun updateLibrary(command: UpdateLibraryCommand): SavedLibraryRoot {
        val existing = requireOwnedLibrary(command.id)
        val decision = LibraryMutationDecider.decideUpdate(existing, command)
        return mutationRepository.updateLibrary(command.id, decision.title.value)
    }
