...
class SyncMetadataWorker(
    ...
    fun start() {
        scope.launch(Dispatchers.IO) {
            ...
            while (isActive) {
                try {
                    val bookId = if (valkeyConnection != null) {
    ...
    private suspend fun processSyncJob(bookId: BookId) {
        ...
        either {
            ...
            val ebookEntry = metadataAggregate.editions.find { it.edition.format == BookFormat.EBOOK } ?: return@either
            val ebook = ebookEntry.edition
            val metadata = metadataAggregate.metadata
            
            val filePath = storageService.resolve(ebook.path)
            if (Files.exists(filePath)) {
                val updates = EpubMetadataUpdates(
