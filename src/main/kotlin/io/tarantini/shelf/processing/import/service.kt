...
import io.tarantini.shelf.processing.import.domain.ScanDirectoryCommand
...
interface ImportService {
    ...
    context(_: RaiseContext, auth: JwtContext)
    suspend fun scanDirectory(command: ScanDirectoryCommand)
...
fun importService(
    ...
    object : ImportService {
                ...
                try {
                    if (job.deleteSource) {
                        install({ job.sourcePath }) { path, _ ->
                                ...
                                path.deleteIfExists()
                                logger.debug { "Worker-$workerId: Deleted source file." }
                            }) { e ->
                                logger.error(e) {
                                    "Worker-$workerId: Failed to delete source file."
                                }
                            }
                        }
                    }

                    logger.info { "Worker-$workerId: Processing background import job." }
                            ...
                            .fold(
                                ifLeft = { error ->
                                    ...
                                    observability
                                        ...
                                        .increment()
                                    logger.error {
                                        "Worker-$workerId: Background import failed: $msg"
                                    }
                                },
                                ifRight = {
                        ...
                        logger.error(e) { "Worker-$workerId: Unexpected background import error." }
                    }
                } finally {
        ...
        context(_: RaiseContext, auth: JwtContext)
        override suspend fun scanDirectory(command: ScanDirectoryCommand) {
            withContext(Dispatchers.IO) {
                ...
                logger.info { "Starting import scan." }
                ...
                scope.launch(Dispatchers.IO) {
                        ...
                        Files.walk(rootPath).use { stream ->
                            ...
                            logger.info { "Import scan found ${files.size} supported files." }
                            ...
                            files.forEach { file ->
                                ...
                                logger.debug { "Queued file for import scan." }
                            }
        ...
        private suspend fun prepareImport(
            ...
            logger.debug { "Processing metadata for import payload." }
            ...
            logger.debug { "Metadata processed successfully." }
            ...
            logger.info { "Saving imported book file." }
            ...
            if (processedWithHash.coverImage != null) {
                ...
                logger.info { "Saving imported cover image." }
        ...
        private suspend fun importToStagingInternal(
            ...
            logger.info { "Starting import to staging." }
            ...
            logger.info { "Created staged import record." }
