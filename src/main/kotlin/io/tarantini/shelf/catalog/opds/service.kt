...
import io.tarantini.shelf.catalog.book.BookPagingProvider
...
private class DefaultOpdsService(
    private val bookPagingProvider: BookPagingProvider,
    ...
    override suspend fun getAllBooksFeed(page: Int, size: Int): OpdsFeed {
        val updated = now()
        val bookPage = bookPagingProvider.getBooksPage(page, size, format = BookFormat.EBOOK)
        ...
        val entries =
            authors.map { author ->
                ...
                OpdsEntry(
                    ...
                    title = author.name,
                    updated = updated,
                    content = OpdsContent(type = "text", value = "${author.ebookCount} e-books"),
                    link =
                        listOf(
                            OpdsLink(
    ...
    context(_: RaiseContext)
    override suspend fun getAuthorBooksFeed(authorId: AuthorId, page: Int, size: Int): OpdsFeed {
        val updated = now()
        val author = authorService.getAuthor(authorId)
        val bookPage =
            bookPagingProvider.getBooksByAuthorPage(authorId, page, size, format = BookFormat.EBOOK)
        val entries = bookPage.items.map { mapBookToEntry(it, updated) }
        ...
        val entries =
            seriesList.map { series ->
                ...
                OpdsEntry(
                    ...
                    title = series.name,
                    updated = updated,
                    content = OpdsContent(type = "text", value = "${series.ebookCount} e-books"),
                    link =
                        listOf(
                            OpdsLink(
    ...
    context(_: RaiseContext)
    override suspend fun getSeriesBooksFeed(seriesId: SeriesId, page: Int, size: Int): OpdsFeed {
        val updated = now()
        val series = seriesService.getSeries(seriesId)
        val bookPage =
            bookPagingProvider.getBooksBySeriesPage(seriesId, page, size, format = BookFormat.EBOOK)
        val entries = bookPage.items.map { mapBookToEntry(it, updated) }
    ...
    override suspend fun searchBooksFeed(query: String, page: Int, size: Int): OpdsFeed {
        ...
        val aggregates = bookAggregateProvider.getBooksAggregates(bookIds)
