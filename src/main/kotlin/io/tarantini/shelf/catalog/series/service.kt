...
interface SeriesModifier {
    context(_: RaiseContext)
    suspend fun createSeries(command: CreateSeriesCommand): SavedSeriesRoot

    context(_: RaiseContext)
    suspend fun updateSeries(command: UpdateSeriesCommand): SavedSeriesRoot
...
fun seriesService(
    ...
    mutationRepository: SeriesMutationRepository = seriesMutationRepository(seriesQueries),
): SeriesService =
    SeriesAggregateService(seriesQueries, bookQueries, storageService, mutationRepository)

private class SeriesAggregateService(
    private val seriesQueries: SeriesQueries,
    private val bookQueries: BookQueries,
    private val storageService: StorageService,
    private val mutationRepository: SeriesMutationRepository,
) : SeriesService {
    context(_: RaiseContext)
    ...
    override suspend fun searchSeriesFuzzy(name: String): List<SeriesSummary> =
        withContext(Dispatchers.IO) { raise(SeriesFuzzySearchDisabled) }
    ...
    context(_: RaiseContext)
    override suspend fun createSeries(command: CreateSeriesCommand): SavedSeriesRoot {
        val decision = SeriesMutationDecider.decideCreate(command)
        return mutationRepository.insertSeries(decision.title.value)
    }

    context(_: RaiseContext)
    override suspend fun updateSeries(command: UpdateSeriesCommand): SavedSeriesRoot {
        val existing = mutationRepository.getSeriesById(command.id)
        val decision = SeriesMutationDecider.decideUpdate(existing, command)
        return mutationRepository.updateSeries(decision.title.value, command.id)
    }
