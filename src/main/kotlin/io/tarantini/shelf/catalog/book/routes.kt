                ...
                respond({
                    val format =
                        resource.format
                            ...
                            ?.let { runCatching { BookFormat.valueOf(it.uppercase()) }.getOrNull() }
                    val status =
                        resource.status
                            ?.takeIf { it.isNotBlank() }
                            ?.let { runCatching { ReadStatus.valueOf(it.uppercase()) }.getOrNull() }
                    val page =
                        bookPagingProvider.getBooksPage(
                            ...
                            titleQuery = resource.title?.takeIf { it.isNotBlank() },
                            authorQuery = resource.author?.takeIf { it.isNotBlank() },
                            seriesQuery = resource.series?.takeIf { it.isNotBlank() },
                            status = status,
                            format = format,
                        )
