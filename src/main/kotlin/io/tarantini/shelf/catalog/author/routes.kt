...
import io.tarantini.shelf.catalog.author.domain.toUpdateCommand
import io.tarantini.shelf.catalog.book.AuthorBookSummaryProvider
...
fun Route.authorRoutes(
    ...
    bookSummaryProvider: AuthorBookSummaryProvider,
        ...
        sharedCatalogMutation(jwtService) {
            respond({
                call.receive<Request<AuthorRequest>>().data.let {
                    authorService.updateAuthor(it.toUpdateCommand())
                }
            })
        }
    }
            ...
            with(auth) {
                respond({
                    ...
                    activityService.enrichBookSummaries(
                        bookSummaryProvider.getBookSummariesForAuthor(authorId = authorId)
                    )
                })
            }
        }
