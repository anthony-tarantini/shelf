...
interface AuthorModifier {
    context(_: RaiseContext)
    suspend fun createAuthor(command: CreateAuthorCommand): SavedAuthorRoot

    context(_: RaiseContext)
    suspend fun updateAuthor(command: UpdateAuthorCommand): SavedAuthorRoot
...
fun authorService(
    ...
    mutationRepository: AuthorMutationRepository = authorMutationRepository(authorQueries),
): AuthorService = AuthorAggregateService(sqlDriver, authorQueries, bookQueries, mutationRepository)

private class AuthorAggregateService(
    private val sqlDriver: SqlDriver,
    private val authorQueries: AuthorQueries,
    private val bookQueries: BookQueries,
    private val mutationRepository: AuthorMutationRepository,
) : AuthorService {
    context(_: RaiseContext)
    override suspend fun getAuthorsPage(page: Int, size: Int, sortBy: String, sortDir: String) =
        withContext(Dispatchers.IO) { authorQueries.getAuthorsPage(page, size, sortBy, sortDir) }
    ...
    context(_: RaiseContext)
    override suspend fun createAuthor(command: CreateAuthorCommand): SavedAuthorRoot {
        val decision = AuthorMutationDecider.decideCreate(command)
        return mutationRepository.createAuthor(decision.name.value)
    }

    context(_: RaiseContext)
    override suspend fun updateAuthor(command: UpdateAuthorCommand): SavedAuthorRoot {
        val existing = mutationRepository.getAuthorById(command.id)
        val decision = AuthorMutationDecider.decideUpdate(existing, command)
        return mutationRepository.updateAuthor(command.id, decision.name.value)
    }
