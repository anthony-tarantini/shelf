...
import io.tarantini.shelf.user.identity.domain.toSetupCommand
...
fun Route.userRoutes(userService: UserService, jwtService: JwtService) {
        ...
        respond(
            {
                ...
                val (token, user) =
                    userService.setup(
                        NewUserRequest(req.email, req.username, req.password).toSetupCommand()
                    )
    ...
    post<UsersResource.Login> {
        respond({
            val req = call.receive<Request<UserRequest>>().data
            val (token, user) =
                userService.login(LoginUserRequest(req.email, req.password).toCommand())
            UserWithToken(user, UserToken(token.value))
        })
    }
        ...
        respond(
            {
                ...
                val (token, user) =
                    userService.register(
                        NewUserRequest(req.email, req.username, req.password).toRegisterCommand()
                    )
                UserWithToken(user, UserToken(token.value))
            },
            HttpStatusCode.Created,
